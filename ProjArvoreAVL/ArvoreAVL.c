...
int VerificaAVL(ArvAVL raiz) {
    ...
    int altura_esq = VerificaAVL(raiz->esq);
    if (altura_esq == -2) { 
        return -2; 
    }
    int altura_dir = VerificaAVL(raiz->dir);
    if (altura_dir == -2) { 
        return -2;
    }
    // fb = Fator de Balanceamento
    int fb = altura_esq - altura_dir;
    if (fb < -1 || fb > 1) {
        return -2;
    }
    return maior(altura_esq, altura_dir) + 1;
}

// Auxiliar pra transformaAVL
ArvAVL ConstroiAVL(int vetor[], int inicio, int fim) {
    if (inicio > fim) 
        return NULL;
    int meio = inicio + (fim - inicio) / 2;
    ArvAVL novo_no = CriaNoAVL(vetor[meio]); 
    novo_no->esq = ConstroiAVL(vetor, inicio, meio - 1);
    novo_no->dir = ConstroiAVL(vetor, meio + 1, fim);
    novo_no->altura = maior(AlturaNo(novo_no->esq), AlturaNo(novo_no->dir)) + 1;

    return novo_no;
}

// Ex. 8 - Faça uma função Arv* transforma(Arv *raiz) que dada uma árvore binária 
// de busca qualquer, retorna uma nova árvore AVL.
ArvAVL transformaAVL(ArvAVL *raiz) {
    if (*raiz == NULL) 
        return NULL;
    int tamanho = ContaNos(*raiz); 
    int *vetor = (int *)malloc(tamanho * sizeof(int));
    if (vetor == NULL) 
        return NULL; 
    int indice = 0;
    ArvAVL nova_raiz_avl = ConstroiAVL(vetor, 0, tamanho - 1);
    free(vetor);
    return nova_raiz_avl; 
}